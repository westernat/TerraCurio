...
import org.confluence.terra_curio.common.component.primitive.EntityTypeValue;
...
//            BEE_CLOAK = registerCurio("bee_cloak", BeeCloak::new), // 蜜蜂斗篷
            BLACK_BELT = registerCurio("black_belt", builder -> builder.attribute(TCAttributes.getDodgeChance(), "dodge", 0.1, ADD_VALUE)), // 黑腰带
            /* 天界徽章 */
            /* 月光护身符 */
...
//            CROSS_NECKLACE = registerCurio("cross_necklace", CrossNecklace::new), // 十字项链
            RANGER_EMBLEM = registerCurio("ranger_emblem", builder -> builder.noTooltip().attribute(TCAttributes.getRangedDamage(), "ranged_damage", 0.15, ADD_MULTIPLIED_TOTAL)), // 游侠徽章
            ...
            SORCERER_EMBLEM = registerCurio("sorcerer_emblem", builder -> builder.noTooltip().attribute(TCAttributes.getMagicDamage(), "magic_damage", 0.15, ADD_MULTIPLIED_TOTAL)), // 巫士徽章
            ...
            EYE_OF_THE_GOLEM = registerCurio("eye_of_the_golem", builder -> builder.noTooltip().attribute(TCAttributes.getCriticalChance(), "critical_chance", 0.1, ADD_VALUE)), // 石巨人之眼
            ...
            FIRE_GAUNTLET = registerCurio("fire_gauntlet", builder -> builder
                    .accessories(units(AUTO_ATTACK, FIRE_ATTACK))
                    .attribute(Attributes.ATTACK_DAMAGE, "attack_damage", 0.12, ADD_MULTIPLIED_TOTAL)
                    .attribute(Attributes.ATTACK_SPEED, "attack_speed", 0.12, ADD_MULTIPLIED_TOTAL)
                    .attribute(Attributes.ATTACK_KNOCKBACK, "attack_knockback", 1.0, ADD_MULTIPLIED_TOTAL)
...
//            SHACKLE = registerCurio("shackle", Shackle::new); // 脚镣
    ...
    public static final Supplier<BaseCurioItem> ROYAL_GEL = registerCurio("royal_gel", builder -> builder.rarity(ModRarity.EXPERT).accessories(of(LIVING_IGNORE, new EntityTypeValue(EntityType.SLIME)))), // 皇家凝胶
            SHIELD_OF_CTHULHU = registerCurio("shield_of_cthulhu", builder -> builder.rarity(ModRarity.EXPERT).accessories(units(CTHULHU))), // 克苏鲁护盾
            WORM_SCARF = registerCurio("worm_scarf", builder -> builder.rarity(ModRarity.EXPERT).accessories(of(INJURY_FREE, new FloatValue(0.17F)))), // 蠕虫围巾
            BRAIN_OF_CONFUSION = registerCurio("brain_of_confusion", builder -> builder.rarity(ModRarity.EXPERT).accessories(units(BRAIN))), // 混乱之脑
            HIVE_PACK = registerCurio("hive_pack", builder -> builder.rarity(ModRarity.EXPERT).accessories(units(HIVE))), // 蜂巢背包
    /* 骨头手套 */
    /* 骸骨头盔 */
    /* 挥发明胶 */
    /* 孢子囊 */
    /* 闪亮石 */
    /* 翱翔徽章 */
    GRAVITY_GLOBE = registerCurio("gravity_globe", builder -> builder.accessories(units(GRAVITY))); // 重力球
