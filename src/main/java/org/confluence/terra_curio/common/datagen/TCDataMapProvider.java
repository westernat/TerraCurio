...
public class TCDataMapProvider extends PortDataMapProvider {
    ...
    protected void gather(HolderLookup.Provider provider) {
        ...
        add(TCItems.ANGLER_EARRING, helper -> {
            helper.entry(TCItems.ATTRIBUTES, AttributeModifiersValue.simple(Attributes.LUCK, helper.asId(), 10, PortAttributeModifier.PortOperation.ADD_VALUE));
            helper.of(TCItems.COMPONENTS, List.of());
        });
        add(TCItems.MAGMA_STONE, helper -> helper.unit(TCItems.FIRE$ATTACK));
        add(TCItems.SHACKLE, helper -> helper.entry(TCItems.ATTRIBUTES, AttributeModifiersValue.simple(Attributes.ARMOR, helper.asId(), 1, PortAttributeModifier.PortOperation.ADD_VALUE)));
        add(TCItems.BASE_POINT, helper -> {
            ...
            helper.entry(TCItems.ATTRIBUTES, AttributeModifiersValue.builder()
                    .add(Attributes.MOVEMENT_SPEED, id, 0.16, PortAttributeModifier.PortOperation.ADD_MULTIPLIED_TOTAL)
                    .add(PortAttributesExtension.fallDamageMultiplier().value(), id, -100, PortAttributeModifier.PortOperation.ADD_VALUE)
                    .add(PortAttributesExtension.jumpStrength().value(), id, 1, PortAttributeModifier.PortOperation.ADD_MULTIPLIED_TOTAL)
                    .add(PortAttributesExtension.stepHeight().value(), id, 0.5, PortAttributeModifier.PortOperation.ADD_VALUE)
                    .build());
        });
        add(TCItems.EVERLASTING, helper -> {
            ...
            helper.entry(TCItems.ATTRIBUTES, AttributeModifiersValue.simple(LibAttributes.getDodgeChance().value(), helper.asId(), 0.1, PortAttributeModifier.PortOperation.ADD_VALUE));
        });
    }
