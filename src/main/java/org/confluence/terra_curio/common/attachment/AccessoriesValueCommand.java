...
public class AccessoriesValueCommand {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("terra_curio").requires(sourceStack -> sourceStack.permissions().hasPermission(Permissions.COMMANDS_GAMEMASTER))
                .then(Commands.argument("type", ValueTypeArgument.type()).executes(context -> {
                    Identifier id = ValueTypeArgument.getId(context, "type");
    ...
    public static class ValueTypeArgument implements ArgumentType<Identifier> {
        ...
        public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
            if (AVAILABLE == null) {
                AVAILABLE = ValueType.TYPES.keySet().stream().map(Identifier::toString).sorted().toList();
            }
