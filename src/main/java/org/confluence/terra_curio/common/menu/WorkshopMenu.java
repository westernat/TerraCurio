...
import org.confluence.lib.network.SyncMultiRecipesPacketS2C;
...
public class WorkshopMenu extends AbstractContainerMenu implements IMultiRecipeMenu<WorkshopRecipe> {
    ...
    public void slotsChanged(Container pContainer) {
        access.execute((_, _) -> {
            if (player instanceof ServerPlayer serverPlayer) {
                setRecipes(serverPlayer.level().getServer().getRecipeManager().recipeMap().getRecipesFor(getRecipeType(), input, player.level()).toList());
                ...
                serverPlayer.connection.send(new ClientboundBundlePacket(List.of(
                        new ClientboundContainerSetSlotPacket(containerId, incrementStateId(), 0, itemStack),
                        SyncMultiRecipesPacketS2C.makePacket(this)
                )));
            }
        });
    }
    ...
    public ItemStack quickMoveStack(Player pPlayer, int pIndex) {
        ...
        if (slot.hasItem()) {
            ...
            if (pIndex == 0) { // resultSlot
                access.execute((_, _) -> slotItem.getItem().onCraftedBy(slotItem, pPlayer));
    ...
    @Override
    public void setRecipes(List<RecipeHolder<WorkshopRecipe>> list) {
        this.recipes = list;
        if (selectedRecipeIndex.get() >= recipes.size()) selectedRecipeIndex.set(recipes.size() - 1);
    }

    @Override
    public List<RecipeHolder<WorkshopRecipe>> getRecipes() {
        return recipes;
    }
