    ...
    public Multimap<Attribute, AttributeModifier> getAttributeModifiers(SlotContext slotContext, UUID uuid, ItemStack stack) {
        if (ATTRIBUTES == null) {
            ATTRIBUTES = ImmutableMultimap.of(
                ModAttributes.getCriticalChance(), new AttributeModifier(CRIT_UUID, "Sniper Scope", ModConfigs.SNIPER_SCOPE_CRITICAL_CHANCE.get(), AttributeModifier.Operation.ADDITION),
                ModAttributes.getRangedDamage(), new AttributeModifier(RANGED_UUID, "Sniper Scope", ModConfigs.SNIPER_SCOPE_RANGED.get(), AttributeModifier.Operation.MULTIPLY_TOTAL)
            );
        }
        return ATTRIBUTES;
    }
