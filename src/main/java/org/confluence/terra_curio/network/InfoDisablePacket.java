...
public record InfoDisablePacket(boolean[] disables) implements IPacket {
    ...
    public void c2s(ServerPlayer player) {
        ...
        for (int i = 0; i < ARRAY_LENGTH; i++) {
            arrayTag.setTag(i, ByteTag.valueOf(disables[i]));
        }
    ...
    public static void sendToClient(ServerPlayer player) {
        byte[] bytes = LibUtils.getOrCreatePersistedData(player).getByteArray("terra_curio:info_disable").orElse(null);
        if (bytes == null || bytes.length != ARRAY_LENGTH) return;
    ...
    public static void sendToServer(boolean[] disables) {
        ClientPacketDistributor.sendToServer(new InfoDisablePacket(disables));
    }
}
