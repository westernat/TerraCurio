...
public abstract class CombineRule<T, V extends PrimitiveValue<T>> {
    public static final Map<Identifier, CombineRule<?, ?>> RULES = new Hashtable<>();

    public static final Codec<CombineRule<?, ?>> CODEC = Identifier.CODEC.xmap(RULES::get, CombineRule::id); // 暂未使用
    ...
    public static <T, V extends PrimitiveValue<T>> CombineRule<T, V> register(BiFunction<T, T, T> combineFunc, Identifier id) {
