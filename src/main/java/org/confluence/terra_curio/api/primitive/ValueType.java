...
public class ValueType<T, V extends PrimitiveValue<T>> {
    ...
    public static final Map<Identifier, ValueType<?, ? extends PrimitiveValue<?>>> TYPES = new Hashtable<>();
    public static final Codec<ValueType<?, ? extends PrimitiveValue<?>>> CODEC = Identifier.CODEC.xmap(TYPES::get, ValueType::key);
    public static final StreamCodec<ByteBuf, ValueType<?, ?>> STREAM_CODEC = Identifier.STREAM_CODEC.map(TYPES::get, ValueType::key);
    ...
    // 不允许从外部创建，这样是不安全的
    private ValueType(Identifier key, CombineRule<T, V> combineRule, T defaultValue, Function<T, V> factory) {
        this.key = key;
    ...
    private static void registerType(Identifier id, ValueType<?, ? extends PrimitiveValue<?>> type) {
    ...
    public static <T, V extends PrimitiveValue<T>> ValueType<T, V> create(Identifier id, CombineRule<T, V> combineRule, Codec<V> codec, T defaultValue, Function<T, V> factory) {
    ...
    public static class IntegerType extends ValueType<Integer, IntegerValue> {
        private IntegerType(Identifier key, CombineRule<Integer, IntegerValue> combineRule, Integer defaultValue) {
            super(key, combineRule, defaultValue, IntegerValue::new);
        }

        public static IntegerType of(Identifier id, CombineRule<Integer, IntegerValue> combineRule, int defaultValue) {
    ...
    public static class FloatType extends ValueType<Float, FloatValue> {
        private FloatType(Identifier key, CombineRule<Float, FloatValue> combineRule, float defaultValue) {
            super(key, combineRule, defaultValue, FloatValue::new);
        }

        public static FloatType of(Identifier id, CombineRule<Float, FloatValue> combineRule, float defaultValue) {
    ...
    public static class UnitType extends ValueType<Unit, UnitValue> {
        private UnitType(Identifier key) {
            super(key, UnitValue.GET_SELF, Unit.INSTANCE, UnitValue.UNIT_2_VALUE);
        }

        public static UnitType of(Identifier id) {
