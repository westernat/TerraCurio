...
public final class TCClientPacketHandler {
    ...
    public static boolean canShowNeptunesShell(LivingEntity living) {
        return ((hasNeptunesShell && living.getClass() == LocalPlayer.class) || (living.getClass() == RemotePlayer.class && remoteNeptuneShell.get(living.getId()))) && living.isInWall();
    }
    ...
    public static int getLuminance(Entity entity) {
        ...
        if (ret < 0) { // 只能在水下发光
            return entity.isEyeInFluid(Tags.Fluids.WATER) ? -ret : 0; // confluence mixin here
        }
