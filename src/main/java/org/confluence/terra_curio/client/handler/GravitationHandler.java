...
public final class GravitationHandler {
    ...
    public static void handle(LocalPlayer player) {
        ...
        if (TCKeyBindings.FLIP_GRAVITATION.get().isDown()) {
            if (!keyDown) {
                ...
                ClientPacketDistributor.sendToServer(new GravitationPacketC2S(shouldRot));
            }
            keyDown = true;
        } else {
            keyDown = false;
    ...
    public static void force(LocalPlayer player) {
        ...
        if (!shouldRot) {
            shouldRot = true;
            player.resetFallDistance();
            ClientPacketDistributor.sendToServer(new GravitationPacketC2S(true));
        }
    }
    ...
    public static void expire() {
        if (shouldRot) {
            shouldRot = false;
            ClientPacketDistributor.sendToServer(new GravitationPacketC2S(false));
        }
    }
    ...
    public static void tryExpire(LocalPlayer player) {
        if (player.getY() > player.level().getMaxY()) {
            expire();
        }
    }
